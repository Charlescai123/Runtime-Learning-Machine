...
class Trainer:
    ...
    def interaction_step(self, mode=None):
        ...
        self.logger.update_logs(
            ...
            energy=energy_value(state=np.array(self.cartpole.state[:4]), p_mat=MATRIX_P)
        )
        ...
        for ep_i in range(int(self.agent_params.max_training_episodes)):
            ...
            for step in range(self._max_steps_per_episode):
                ...
                if self.ha_params.teacher_correct is False and ha_flag is True:
                    ...
                    logger.debug("HP-Student doesn't learn from HA-Teacher, skip model updating...")
                else:
    ...
    def get_terminal_action(self, state, mode=None):
        ...
        terminal_action, action_mode = self.coordinator.get_terminal_action(hp_action=hp_action,
                                                                            ha_action=ha_action,
                                                                            plant_state=s,
        ...
        if action_mode == ActionMode.TEACHER:
            if self.ha_teacher.teacher_correct:  # Correction from HA-Teacher
