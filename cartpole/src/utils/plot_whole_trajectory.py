...
def plot_inference_phase_drl(trajectories, name, label, color=[0, 0.4470, 0.7410]):
    ...
    for i in range(len(trajectories) - 1):
        ...
        plt.plot([x, x_next], [y, y_next], linewidth=3.5, color=color)
    xx = trajectories[-2][0]
    yy = trajectories[-2][2]
    ...
    h4, = plt.plot(trajectories[-1][0], trajectories[-1][2], color='black', marker='*', markersize=20,
                   mew=2)  # end state
    ...
    plt.legend(loc="upper right", markerscale=4, handlelength=1.5, handletextpad=0.5, fontsize=30)


if __name__ == '__main__':
    fig = plt.figure(figsize=(12, 10))
    plot_safety_envelope(p_mat=MATRIX_P / 0.6 * 0.3)
    ...
    file_name11 = f'data/fric-20-0.003/trajectory-chi-0.txt'
    file_name12 = f'data/fric-20-0.003/trajectory-chi-0.15.txt'
    file_name13 = f'data/fric-20-0.003/trajectory-chi-0.3.txt'

    file_name21 = f'data/fric-25-0.003/trajectory-chi-0.txt'
    file_name22 = f'data/fric-25-0.003/trajectory-chi-0.15.txt'
    file_name23 = f'data/fric-25-0.003/trajectory-chi-0.3.txt'

    file_name31 = f'data/fric-25-0.006/trajectory-chi-0.txt'
    file_name32 = f'data/fric-25-0.006/trajectory-chi-0.15.txt'
    file_name33 = f'data/fric-25-0.006/trajectory-chi-0.3.txt'


    if order == 1:
        cnt11 = np.loadtxt(file_name11)
        plot_inference_phase_drl(cnt11, name=1, color='blue', label='$\\chi$=0')
        cnt12 = np.loadtxt(file_name12)
        plot_inference_phase_drl(cnt12, name=2, color='green', label='$\\chi$=0.15')
        cnt13 = np.loadtxt(file_name13)
        plot_inference_phase_drl(cnt13, name=3, color='red', label='$\\chi$=0.3')
    # plt.ylabel('$\\theta$', fontsize=35)
    elif order == 2:
        cnt21 = np.loadtxt(file_name21)
        plot_inference_phase_drl(cnt21, name=1, color='blue', label='$\\chi$=0')
        cnt22 = np.loadtxt(file_name22)
        plot_inference_phase_drl(cnt22, name=2, color='green', label='$\\chi$=0.15')
        cnt23 = np.loadtxt(file_name23)
        plot_inference_phase_drl(cnt23, name=3, color='red', label='$\\chi$=0.3')

    elif order == 3:
        cnt31 = np.loadtxt(file_name31)
        plot_inference_phase_drl(cnt31, name=1, color='blue', label='$\\chi$=0')
        cnt32 = np.loadtxt(file_name32)
        plot_inference_phase_drl(cnt32, name=2, color='green', label='$\\chi$=0.15')
        cnt33 = np.loadtxt(file_name33)
        plot_inference_phase_drl(cnt33, name=3, color='red', label='$\\chi$=0.3')

    # fig.savefig(f"phase1.pdf")
    # fig.savefig(f"phase2.pdf")
    fig.savefig(f"data/phase{order}.pdf")
