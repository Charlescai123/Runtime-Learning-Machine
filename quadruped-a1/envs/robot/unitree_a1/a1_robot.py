...
from envs.robot.state_estimator import a1_robot_state_estimator
from envs.robot.unitree_a1.motors import MotorControlMode
...
class A1Robot(a1.A1):
    """Class for interfacing with A1 physical robot."""
    ...
    def __init__(
        ...
        print("robot_interface is initialized.")
    ...
    def _apply_action(self,
        ...
        # Torque Mode
            ...
            action.desired_torque = np.clip(action.desired_torque,
                                            self.motor_group.motor_min_torques,
                                            self.motor_group.motor_max_torques)
            for motor_id in range(self.num_motors):
        ...
        # Hybrid Mode
            ...
            # clip desired torque for real world robot
            action.desired_torque = np.clip(action.desired_torque,
                                            self.motor_group.motor_min_torques,
                                            self.motor_group.motor_max_torques)
    ...
    def reset(self, hard_reset: bool = False, reset_time=1.5):
        ...
        # super(A1Robot, self).reset(hard_reset, num_reset_steps=0) # this is for the simulation
        ...
        for _ in range(10):
            ...
            time.sleep(0.001)
            self._receive_observation()
        print("About to reset the robot, sending zero command for 10 times...")
        ...
        for leg_id in range(4):
            self.update_foot_contact_force_threshold(leg_id, stand_foot_forces[leg_id] * 0.8)
