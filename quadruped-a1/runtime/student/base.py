...
class BaseAgent:
    def __init__(self, params: BaseAgentParams, shape_observations=5, shape_action=1,
                 on_edge=False):
        self.params = params
    ...
    def build_critic(self, name):
        ...
        input_concat = Concatenate(axis=1)([input_observations, input_action])
        ...
        model_critic = Model(inputs=[input_observations, input_action], outputs=output_value,  name=name + 'critic')
    ...
    def build_actor(self, name):
        ...
        dense1_actor = Dense(self.params.actor_dense1, activation='relu', name=name + 'dense1_actor')(input_observations)
        ...
        model_actor = Model(inputs=input_observations, outputs=output_action, name=name + 'actor')
        return model_actor
    ...
    def get_exploration_action(self, observations):
        ...
        action = self.actor(observations).numpy().squeeze()
    ...
    def get_exploitation_action_target(self, observations):
        """
        ...
        """
        action = self.actor_target(observations)
        return action
